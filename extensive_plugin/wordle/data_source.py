...
class Wordle(object):
    ...
    def draw(self) -> BytesIO:
        ...
        for i in range(self.rows):
            word_temp = list(self.word_lower)  # 临时变量
            result = []  # 灰-0 绿-1 黄-2 白-3 不确定-4
            colors = [self.wrong_color, self.correct_color, self.exist_color, self.bg_color]
            """
                先匹配相同位置
                再判断是否存在但位置不同
            """
            ...
            for j in range(self.length):
                ...
                board.paste(self.draw_block(colors[result[j]], letter), (x, y))
